...
6. `Agent Runner`
   ...
   - Launches the Claude Code CLI subprocess (one per turn) and streams `stream-json` events.
...
- Claude Code CLI executable that supports `--output-format stream-json`.
...
- `session_id` (string, native Claude Code session id)
- `thread_id` (string, equal to the Claude Code `session_id`)
- `turn_id` (string, per-turn counter within the worker)
...
- `Session ID`
  - Use the native Claude Code `session_id` reported in the `system`/`init` `stream-json`
    event.
  - `thread_id` is the same `session_id`, reused for continuation turns via `--resume`.
  - `turn_id` is a per-turn counter within the worker lifetime.
  - Keep the emitted `session_id` stable across continuation turns of one worker run.
...
Claude Code has no separate `approval_policy` / `thread_sandbox` / `turn_sandbox_policy`
config surface. Permission and sandbox posture is expressed as CLI flags inside
`claude.command` (for example `--permission-mode`, `--allowedTools`,
`--disallowedTools`, or `--dangerously-skip-permissions`). Implementations MAY validate
the assembled command locally if they want stricter startup checks.
...
- `resume_across_turns` (boolean)
  ...
  - When `true`, continuation turns re-attach to the same session by appending
    `--resume <session_id>` to `command` once a session id is known.
...
CLI in its `stream-json` mode. The Claude Code CLI for the targeted version is the source of truth
for command-line flags, `stream-json` event shapes, and transport framing.

Protocol source of truth:

- Implementations MUST invoke the Claude Code CLI in a way that is valid for the targeted CLI
  version.
- Implementations MUST consult the targeted Claude Code CLI documentation instead of treating this
  specification as a protocol schema.
- If this specification appears to conflict with the targeted Claude Code CLI behavior, the CLI
  controls command-line shape, `stream-json` event shape, and transport behavior.
...
- Claude Code runs as **one subprocess per turn**. There is no persistent server process.
- Each turn spawns a fresh CLI process, writes the rendered prompt to its `stdin`, and reads
  newline-delimited `stream-json` events from its `stdout` until a terminal `result` event.
- Session continuity across turns is provided by `--resume <session_id>`, not by a long-lived
  process.
...
- Prompt input: the rendered prompt is written to the process `stdin`, then `stdin` is closed
- Transport/framing: newline-delimited `stream-json` events on `stdout`

Notes:

- The default command is `claude -p --output-format stream-json --verbose`.
- Permission/sandbox posture, cwd, prompt input, and OPTIONAL tool declarations are supplied using
  Claude Code CLI flags (see Section 5.3.6) inside `claude.command`.
- On continuation turns, when `claude.resume_across_turns` is `true` and a session id is known, the
  runtime appends `--resume <session_id>` to `claude.command`.

RECOMMENDED additional process settings:

- Max line size: 10 MB (for safe buffering of large `stream-json` lines)

### 10.2 Session Startup Responsibilities

Reference: https://docs.anthropic.com/en/docs/claude-code/sdk (Claude Code headless / `stream-json` mode)

Startup MUST follow the targeted Claude Code CLI contract. Claude Code creates a session
implicitly on the first `claude -p` invocation; there is no separate initialize handshake.
Symphony additionally requires the client to:

- Spawn the CLI subprocess with the per-issue workspace as `cwd`.
- Treat the session as pending until the `session_id` arrives; return a placeholder session id
...
- Include issue-identifying metadata, such as `<issue.identifier>: <issue.title>`, in prompt or log
  context.
- Advertise implemented client-side tools using Claude Code's tool mechanism (for example an MCP
  server or `--allowedTools`).

Session identifiers:

- Extract `session_id` from the `system`/`init` `stream-json` event (and, as a fallback, from the
  terminal `result` event).
- Use that `session_id` as both the emitted `session_id` and `thread_id`.
- Track `turn_id` as a per-turn counter within one worker run.
- Reuse the same `session_id` (via `--resume`) for all continuation turns inside one worker run.

### 10.3 Streaming Turn Processing

The client reads `stream-json` events from the subprocess `stdout` until the terminal `result`
event is seen or the process exits. Common event types are `system` (subtype `init`), `assistant`,
`user`, and `result`.

Completion conditions:

- terminal `result` event with a non-error `subtype` / `is_error == false` -> success
- terminal `result` event with an error `subtype` / `is_error == true` -> failure
- stream ends with no `result` event -> failure
- turn timeout (`turn_timeout_ms`) -> failure
- subprocess exit before a `result` event -> failure

Continuation processing:

- If the worker decides to continue after a successful turn, it SHOULD start another turn by
  re-spawning the CLI with `--resume <session_id>` (subject to `claude.resume_across_turns`).
- Each Claude Code subprocess **exits at the end of its turn**; continuity is provided by
  `--resume`, not by keeping a single process alive across turns.

Transport handling requirements:

- Parse one JSON object per line; a run of `MALFORMED_LINE_LIMIT` consecutive unparseable lines
  SHOULD be treated as stream corruption and fail the turn.
- Keep `stdout` `stream-json` handling separate from diagnostic `stderr` handling; drain `stderr`
  into a bounded tail used only for failure diagnostics.

### 10.4 Emitted Runtime Events (Upstream to Orchestrator)

The Claude Code CLI client emits structured events to the orchestrator callback. Each event SHOULD
include:
...
- An implementation MAY expose a limited set of client-side tools to the Claude Code session.
- Current standardized optional tool: `linear_graphql`.
- If implemented, supported tools SHOULD be advertised to the Claude Code session using Claude
  Code's tool mechanism (for example an MCP server or `--allowedTools`).
...
The `Agent Runner` wraps workspace + prompt + Claude Code CLI client.

Behavior:

1. Create/reuse workspace for issue.
2. Build prompt from workflow template.
3. Start the Claude Code session (per-turn subprocess).
4. Forward Claude Code `stream-json` events to orchestrator.
...
- Claude Code reports per-turn usage in the terminal `result` event's `usage` map.
- Extract token counts leniently from the common Claude Code field names:
  - `input_tokens`
  - `output_tokens`
  - `cache_read_input_tokens` and `cache_creation_input_tokens` (cache/input tokens)
- Each `result` event reports usage for that one turn; accumulate turn usage across the worker run
  rather than treating any single event as an absolute thread total.
- Ignore mid-stream `usage` deltas on `assistant` events; the terminal `result` event is the source
  of truth for accumulation.
...
Running Claude Code agents against repositories, issue trackers, and other inputs that can contain
...
- Tightening Claude Code permission and sandbox settings described elsewhere in this specification
...
function run_agent_attempt(issue, attempt, orchestrator_channel):
  ...
  session = agent.start_session(workspace=workspace.path)
  if session failed:
...
### 17.5 Claude Code `stream-json` Client

- Launch command uses workspace cwd and invokes `bash -lc <claude.command>`
- Session startup spawns one subprocess per turn and writes the prompt to `stdin`
- `session_id` is captured from the `system`/`init` event and used to emit `session_started`
- Continuation turns re-attach with `--resume <session_id>` when `claude.resume_across_turns` is
  enabled
- Policy-related launch flags use the implementation's documented permission/sandbox settings
- Request/response read timeout is enforced
- Turn timeout is enforced
- Newline-delimited `stream-json` framing is parsed correctly, and a terminal `result` event ends
  the turn
- Diagnostic `stderr` handling is kept separate from the `stream-json` stdout stream
- A run of consecutive malformed lines is treated as stream corruption and fails the turn
- An error `result` event (`is_error`/error `subtype`) is mapped to turn failure
- Command/file-change approvals are handled according to the implementation's documented policy
- Unsupported dynamic tool calls are rejected without stalling the session
- User input requests are handled according to the implementation's documented policy and do not
  stall indefinitely
- Usage telemetry is extracted from the `result` event's `usage` map
- If client-side tools are implemented, the supported tools are advertised using Claude Code's tool
  mechanism (MCP / allowed tools)
...
- Claude Code subprocess-per-turn client with `stream-json` protocol
- Claude launch command config (`claude.command`, default `claude -p --output-format stream-json --verbose`)
...
- `linear_graphql` client-side tool extension exposes raw Linear GraphQL access through the
  Claude Code session using configured Symphony auth.
...
- The Claude Code CLI is launched over SSH stdio instead of as a local subprocess, so the
